...
class Env:
    ...
    def reset(self):
        ...
        self.request_demand = self.total_demand_at_t[self.ith_demand_at_t]  # 三部分：1.需求类型 2.OD 3. 活动时长 1*4
    ...
    def get_request_demand(self):
        ...
        if len(result) == 0:
            self.req_id_at_t = 0
            return np.zeros((1, 4)).astype(int)  # 如果没有请求 则需求信息全部传回零
        else:
            self.req_id_at_t = result[0, 5]
        ...
        if self.t < self.episode:
            if 0 < action < 13:  # 有分配泊位
                ...
                if self.rule == 1:
                    if action <= 3:
                        if action == 1:
                            self.plm[0].add_req(req_id=self.req_id_at_t, add_type=req_type)
                        elif action == 2:
                            self.plm[0].add_req(req_id=self.req_id_at_t, add_type=req_type)
                        elif action == 3:
                            self.plm[0].add_req(req_id=self.req_id_at_t, add_type=req_type)
                        ...
                        if action == 4:
                            self.plm[1].add_req(req_id=self.req_id_at_t, add_type=req_type)
                        elif action == 5:
                            self.plm[1].add_req(req_id=self.req_id_at_t, add_type=req_type)
                        elif action == 6:
                            self.plm[1].add_req(req_id=self.req_id_at_t, add_type=req_type)
                        ...
                        if action == 7:
                            self.plm[2].add_req(req_id=self.req_id_at_t, add_type=req_type)
                        elif action == 8:
                            self.plm[2].add_req(req_id=self.req_id_at_t, add_type=req_type)
                        elif action == 9:
                            self.plm[2].add_req(req_id=self.req_id_at_t, add_type=req_type)
                        ...
                        if action == 10:
                            self.plm[3].add_req(req_id=self.req_id_at_t, add_type=req_type)
                        elif action == 11:
                            self.plm[3].add_req(req_id=self.req_id_at_t, add_type=req_type)
                        elif action == 12:
                            self.plm[3].add_req(req_id=self.req_id_at_t, add_type=req_type)
                ...
                if action == 13:
                    if this_demand.any() == 0:  # 没有需求
                        self.rewards = 0

                else:  # 有需求但是没分配

                    # 有没有可用泊位
                    mask = self.get_invalid_action()
                    if mask.any() == 0:
                        self.rewards = -10
                    else:
                        self.rewards = -5
            ...
            return self.states, self.rewards, self.termination, [self.total_revenue, self.park_revenue,
                                                                 ...
                                                                 self.travel_cost, self.cruise_cost,
                                                                 self.pl_current_supply]
