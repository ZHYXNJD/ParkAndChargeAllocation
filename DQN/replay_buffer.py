...
class ReplayBuffer(object):
    def __init__(self, args):
        ...
        self.buffer = {'state': np.zeros((self.buffer_capacity, args.state_dim)),
                       ...
                       'this_invalid_action': np.zeros((self.buffer_capacity, args.action_dim)),
                       'next_invalid_action': np.zeros((self.buffer_capacity, args.action_dim)),
                       }

    def store_transition(self, state, action, reward, next_state, this_invalid_action, next_invalid_action, done):
...
class N_Steps_ReplayBuffer(object):
    ...
    def store_transition(self, state, action, reward, next_state, this_invalid_action, next_invalid_action, done):
        transition = (state, action, reward, next_state,done,this_invalid_action, next_invalid_action)
        self.n_steps_deque.append(transition)
        if len(self.n_steps_deque) == self.n_steps:
            state, action, n_steps_reward, next_state, done, this_invalid_action, next_invalid_action,  = self.get_n_steps_transition()
            self.buffer['state'][self.count] = state
            self.buffer['action'][self.count] = action
    ...
    def get_n_steps_transition(self):
        ...
        next_state, done, mask_this, mask_next = self.n_steps_deque[-1][3:]
        n_steps_reward = 0
        for i in reversed(range(self.n_steps)):
            r, s_, d, mask, mask_ = self.n_steps_deque[i][2:]
            n_steps_reward = r + self.gamma * (1 - d) * n_steps_reward
            if d:
                next_state, done, mask_this, mask_next = s_, d, mask, mask_

        return state, action, n_steps_reward, next_state, done,mask_this, mask_next
...
class N_Steps_Prioritized_ReplayBuffer(object):
    ...
    def store_transition(self, state, action, reward, next_state,this_invalid_action,next_invalid_action,done):
        ...
        self.n_steps_deque.append(transition)
        if len(self.n_steps_deque) == self.n_steps:
            state, action, n_steps_reward, next_state, done,mask_this, mask_next = self.get_n_steps_transition()
            self.buffer['state'][self.count] = state
            self.buffer['action'][self.count] = action
            self.buffer['reward'][self.count] = n_steps_reward
            self.buffer['next_state'][self.count] = next_state
