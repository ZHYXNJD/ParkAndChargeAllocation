...
class Runner:
    def __init__(self, args, number, seed):
        ...
        # self.evaluate_env = Env(park_arrival_num=args.park_arrival_num, charge_ratio=args.charge_ratio, rule=args.rule)
    ...
    def evaluate(self):
        ...
        train_loss = []
        episode_steps = 0
        while self.total_steps < self.args.max_evaluate_steps:
            ...
            self.agent.net.load_state_dict(torch.load('test_net.pth'))
            self.agent.net.eval()
        ...
        while self.total_steps < self.args.max_train_steps:
            ...
            while not done:
                ...
                if (episode_steps + 1) % 10 == 0:
                    assign_info.append([self.env.t, self.env.req_id_at_t, self.env.req_t, action])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--forward_scale", type=float, default=0.8, help="used for predict next state feature")
    parser.add_argument("--inverse_scale", type=float, default=0.2, help="used for predict action")
    parser.add_argument("--intrinsic_scale", type=float, default=1, help="used for forward loss")
    parser.add_argument("--use_extrinsic", type=bool, default=True, help="whether use extrinsic reward")
