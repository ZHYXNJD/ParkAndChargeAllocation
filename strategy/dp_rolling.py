...
def dp_rolling(rule, alpha1=0.7, alpha2=0.1, alpha3=0.2):
    ...
    for ith, i in enumerate(request_interval):
        ...
        # 平台收益：预约费 停车费 充电费
        park_revenue = quicksum(
            x_nm[(n, m)] * req_info['revenue'].loc[m] for m in park_index for n in range(N)) + need_adjust * (
                           quicksum(
                               x_na[(n, a)] * req_info['revenue'].loc[a] for a in adj_park_index for n in range(N)))

        char_revenue = quicksum(
            x_nm[(n, m)] * req_info['revenue'].loc[m] for m in charge_index for n in range(N)) + need_adjust * (
                           quicksum(
                               x_na[(n, a)] * req_info['revenue'].loc[a] for a in adj_charge_index for n in range(N)))
        ...
        if need_adjust:
            if X_NA_LAST.shape[1] != 0:
                ...
                m.addConstrs(S_NK[ith][n][k] + quicksum(
                    x_nm[(n, m)] * r_mk[m][k] for m in total_index) + quicksum(
                    x_na[(n, a)] * r_mk[a][k] for a in adj_total_index) <= 1 for n in range(N) for k in
                             range(K))
    ...
    os.chdir('../')
    np.save(f"SNK_info/rdp_{rule}.npy", S_NK)
