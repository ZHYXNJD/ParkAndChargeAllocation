...
class PPO_discrete:
    def __init__(self, args):
        ...
        self.action_space = args.action_dim
    ...
    def choose_action(self, s,invalid_action):
        s = torch.unsqueeze(torch.tensor(s, dtype=torch.float), 0)
        with torch.no_grad():
            probs = self.actor(s)
            mask = torch.tensor(invalid_action,dtype=torch.bool)
            adjust_logits = torch.where(mask, probs, torch.tensor(-1e6))
    ...
    def update(self, replay_buffer, total_steps):
        s,a,a_logprob,r,s_,done,this_mask,next_mask = replay_buffer.numpy_to_tensor()  # Get training data
        """
        ...
        for _ in range(self.K_epochs):
            ...
            for index in BatchSampler(SubsetRandomSampler(range(self.batch_size)), self.mini_batch_size, False):
                ...
                mask = torch.tensor(this_mask[index],dtype=torch.bool)
                adjust_logits = torch.where(mask,probs,torch.tensor((-1e6)))
                dist_now = Categorical(logits=adjust_logits)
