...
class Env:
    def __init__(self, evaluate=False,park_arrival_num=100,charge_ratio=1):
        self.supply_t = []
    ...
    def future_supply(self):
        ...
        pl_ops_future_supply = np.array(current_supply[:,1].tolist()*self.t_next).reshape(pl_num,self.t_next).T.astype(int)
        pl_fcps_future_supply = np.array(current_supply[:,2].tolist()*self.t_next).reshape(pl_num,self.t_next).T.astype(int)
        pl_scps_future_supply = np.array(current_supply[:,3].tolist()*self.t_next).reshape(pl_num,self.t_next).T.astype(int)
    ...
    def step(self, action):
        if self.t < self.episode:
            if action != pl_num:  # 有分配泊位
                ...
                #     demand[2] + 2]
                # 奖励为收益
                self.rewards = self.req_info['revenue'].loc[self.req_id_at_t]
