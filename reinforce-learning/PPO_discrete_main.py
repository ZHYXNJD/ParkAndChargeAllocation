...
from env import Env
import warnings
from strategy.utils import save_reinforcement
    ...
    while total_steps < args.max_train_steps:
        ...
        while not done:
            ...
            if replay_buffer.count == args.batch_size:
                ...
                writer.add_scalar('critic_loss', cri_loss, global_step=int(total_steps/1440))
                writer.add_scalar('dist_entropy', entropy, global_step=int(total_steps/1440))

        episode_steps += 1
        writer.add_scalar('train_episode_reward', env.accumulative_rewards, global_step=int(total_steps/1440))
        ...
        if episode_steps % args.evaluate_freq == 0:
            evaluate_num += 1
            evaluate_reward, result = evaluate_policy(args, env_evaluate, agent, state_norm)
            evaluate_rewards.append(evaluate_reward)
            print("evaluate_num:{} \t evaluate_reward:{} \t".format(evaluate_num, evaluate_reward))
            print(
                f"total rev:{result[0]},park rev:{result[1]},char rev:{result[2]},park refuse:{result[4]},char refuse:{result[5]}")
            print(f"travel cost:{result[6]},cruise cost:{result[7]}")
            writer.add_scalar('evaluate_episode_rewards', evaluate_rewards[-1], global_step=total_steps)
            # Save the rewards
            if evaluate_num % args.save_freq == 0:
                # save rewards
                np.save('../data_train/PPO_discrete_env_{}_number_{}.npy'.format(env_name, number),
                        np.array(evaluate_rewards))
                # save other results  注意rule为1
                save_reinforcement(result, rule=1)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--batch_size", type=int, default=1440, help="Batch size")
    ...
    main(args, env_name=env_name, number=10)
